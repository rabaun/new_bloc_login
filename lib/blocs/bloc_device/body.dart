...
import '../../presentation/table_screen.dart';
import '../bloc_table/bloc_table.dart';
import '../bloc_table/table_event.dart';
...
class _BodyDeviceState extends State<BodyDevice> {
  ...
  List<Widget> _getTitleWidget() {
    return [
      ...
      _getTitleItemWidget('Номер в госреестре', 124),
      _getTitleItemWidget('Заводской номер', 180),
    ];
  }
            ...
            child: InkWell(
                  ...
                  onTap: () {
                    BlocProvider.of<TableBloc>(context)
                        .add(TableEvent.getData(deviceModel: widget.deviceList?[index]));
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => const TableSrceen(),
                      ),
                    );
                  },
                  child: Text("${widget.deviceList?[index].id} ${widget.deviceList?[index].name.toString()}"),
                ),
            onPressed: () {
              Navigator.push(
                context,
